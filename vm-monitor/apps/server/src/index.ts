...
server.listen(CONFIG.port, async () => {
  ...
  if (!CONFIG.disable_integrations) {
    ...
    router.get('/load/firecracker', async (_, res) => {
      res.send(await obs.firecracker.getCurrentValue());
    });
  }
  ...
  io.on('connection', (socket) => {
    ...
    subscriptions.push(
      obs.firecracker.subscribe((firecracker) => {
        socket.emit('firecracker-load', firecracker);
      }),
    );
