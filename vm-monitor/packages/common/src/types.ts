...
export type FirecrackerLoad = {
  vms: FirecrackerVm[];
  base_image_ok: boolean;
  kernel_ok: boolean;
};
...
export type Config = {
  ...
  // Widgets, Labels
  widget_list: ('os' | 'cpu' | 'storage' | 'ram' | 'network' | 'gpu' | 'firecracker')[];
  ...
  // Firecracker Widget
  firecracker_widget_grow: number;
  firecracker_widget_min_width: number;
  firecracker_poll_interval: number;
